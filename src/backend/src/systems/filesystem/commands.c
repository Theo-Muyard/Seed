...
t_ErrorCode	fs_directory_move(t_Manager *manager, const t_Command *cmd)
{
	...
	_tmp = get_parent_path(_payload->old_path);
	_old_parent_dir = recursive_find_sub_directory(_ctx->root, _tmp);
	free(_tmp);
	_tmp = NULL;
	_tmp = get_parent_path(_payload->new_path);
	_new_parent_dir = recursive_find_sub_directory(_ctx->root, _tmp);
	free(_tmp);
