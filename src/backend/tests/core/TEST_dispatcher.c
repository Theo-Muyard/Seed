...
static int	test_dispatcher_init(void)
{
	...
	_manager = calloc(1, sizeof(t_Manager));
	if (NULL == _manager)
		return (print_error("Failed to allocate manager"), 1);

	if (false == dispatcher_init(_manager, 10))
		return (manager_clean(_manager), print_error("Failed to initialize dispatcher with capacity 10"), 1);
	print_success("Dispatcher initialized with capacity 10");

	_dispatcher = _manager->dispatcher;

	if (_dispatcher->count != 0)
		return (manager_clean(_manager), print_error("Initial count should be 0"), 1);
	print_success("Initial count is 0");

	if (_dispatcher->capacity != 10)
		return (manager_clean(_manager), print_error("Capacity should be 10"), 1);
	print_success("Capacity is correctly set to 10");

	if (NULL == _dispatcher->commands)
		return (manager_clean(_manager), print_error("Commands array should be allocated"), 1);
...
static int	test_dispatcher_register(void)
{
	...
	if (false == dispatcher_init(_manager, 5))
		return (manager_clean(_manager), print_error("Failed to initialize dispatcher"), 1);

	_dispatcher = _manager->dispatcher;

	if (false == dispatcher_register(_dispatcher, CMD_WRITING_CREATE_BUFFER, sizeof(t_CmdCreateBuffer), test_handler_success))
		return (manager_clean(_manager), print_error("Failed to register first command"), 1);
	print_success("Registered CMD_WRITING_CREATE_BUFFER");

	if (_dispatcher->count != 1)
		return (manager_clean(_manager), print_error("Count should be 1 after registration"), 1);
	...
	if (false == dispatcher_register(_dispatcher, CMD_WRITING_DELETE_BUFFER, sizeof(t_CmdDestroyBuffer), test_handler_success))
		return (manager_clean(_manager), print_error("Failed to register second command"), 1);
	...
	if (true == dispatcher_register(_dispatcher, CMD_WRITING_DELETE_LINE, sizeof(t_CmdDeleteLine), NULL))
		return (manager_clean(_manager), print_error("Should reject NULL function pointer"), 1);
...
static int	test_dispatcher_exec(void)
{
	...
	if (false == dispatcher_register(_dispatcher, CMD_WRITING_CREATE_BUFFER, sizeof(t_CmdCreateBuffer), test_handler_success))
		return (manager_clean(_manager), print_error("Failed to register command"), 1);

	_cmd.id = CMD_WRITING_CREATE_BUFFER;
	_cmd.payload = NULL;

	if (dispatcher_exec(_manager, &_cmd))
		return (manager_clean(_manager), print_error("Failed to execute registered command"), 1);
	...
	if (false == dispatcher_register(_dispatcher, CMD_WRITING_DELETE_BUFFER, sizeof(t_CmdDestroyBuffer), test_handler_failure))
		return (manager_clean(_manager), print_error("Failed to register failure handler"), 1);

	_cmd.id = CMD_WRITING_DELETE_BUFFER;
	if (!dispatcher_exec(_manager, &_cmd))
		return (manager_clean(_manager), print_error("Should propagate handler failure"), 1);
	print_success("Correctly propagated handler failure");

	_cmd.id = CMD_WRITING_INSERT_LINE;
	if (!dispatcher_exec(_manager, &_cmd))
		return (manager_clean(_manager), print_error("Should fail on unknown command"), 1);
	print_success("Correctly failed on unregistered command");

	if (!dispatcher_exec(NULL, &_cmd))
		return (manager_clean(_manager), print_error("Should fail with NULL manager"), 1);
